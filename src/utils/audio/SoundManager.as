...
//- CONSTRUCTOR	-------------------------------------------------------------------------------------------
		...
		{
			if (!SoundManager._allowInstance || _instance != null)
			{
...
//- PUBLIC & INTERNAL METHODS -----------------------------------------------------------------------------
		...
		public function addExternalSound($path:String, $name:String, $buffer:Number = 1000, $checkPolicyFile:Boolean = false):Boolean
		{
			...
			snd.addEventListener(IOErrorEvent.IO_ERROR, ioError, false, 0, true);
			
			try {
				snd.load(req, new SoundLoaderContext($buffer, $checkPolicyFile) );
		...
		private function ioError(e:IOErrorEvent = null):void {
			Console.log('Check your audio path, error while loading audio')
		}
		...
		public function fadeSound($name:String, $targVolume:Number = 0, $fadeLength:Number = 1):void
		{
			...
			var st:SoundTransform = this._soundsDict[$name].channel.soundTransform;
			
			var gt:GTween = new GTween(snd.channel.soundTransform, $fadeLength, {volume: $targVolume});
			gt.setAssignment(snd.channel, "soundTransform");
		...
		/**
		 * Fades the volume for all sounds in the sound dictionary.
		 * 
		 * @return void
		 */
		public function fadeAllSounds($targVolume:Number = 0, $fadeLength:Number = 1):void
		{
			for (var i:int = 0; i < this._sounds.length; i++)
			{
				var snd:Object = this._sounds[i];
				trace(this._sounds[i]);
				var curTransform:SoundTransform = snd.channel.soundTransform;
				var st:SoundTransform = this._sounds[i].channel.soundTransform;
		 ...
		 * 
		 * @param $name The string identifier of the sound
		 * @param $as_porcentage Specifies if it returns position in milliseconds or porcentage from duration
		 * @return Number The current position of the sound
		 */
		public function getSoundPosition($name:String, $as_porcentage:Boolean = false):Number
		{
			
			return $as_porcentage ? getSoundPosition($name) / getSoundDuration($name) : this._soundsDict[$name].channel.position;
		}
